...
export function Input({ label, icon: Icon, ...props }: InputProps) {
  return (
    <StyledView className="space-y-1.5 w-full">
      <StyledText className="text-[10px] font-bold tracking-widest text-[#a6ada6] uppercase ml-1">
        {label}
      </StyledText>
...
export function Button({ variant = 'primary', size = 'md', className = '', children, onPress }: ButtonProps) {
  ...
  return (
    ...
    >
      <StyledText className={`font-bold ${size === 'sm' ? 'text-xs' : 'text-sm'} ${textVariants[variant]}`}>
        {children}
      </StyledText>
    </StyledTouchableOpacity>
  );
